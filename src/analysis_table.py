...
def find_liv_threshold(current, output_power, liv_threshold_decision_level=2):
    first_der = np.gradient(output_power, current)
    second_der = np.gradient(first_der, current)
    if second_der.max() >= liv_threshold_decision_level:
        threshold_index = [np.argmax(second_der >= liv_threshold_decision_level)]
        threshold_current = float(current[threshold_index])
    else:
        threshold_index = 0
        threshold_current = 0.0
    return threshold_index, threshold_current


def find_liv_rollower(current, output_power):
    rollover_index = np.argmax(output_power)
    rollover_current = float(current[rollover_index])
    rollover_output_power = float(output_power[rollover_index])
    return rollover_index, rollover_current, rollover_output_power
...
def analyze_one_VCSEL_OSA(directory, settings=None):
    ...
    if settings is None:
        with open(Path("templates") / "table.yaml") as fh:
            settings = yaml.safe_load(fh)
    print(directory)
    liv_threshold_decision_level = settings["liv_threshold_decision_level"]


def analyze_one_VCSEL_LIV(directory, settings=None):
    if isinstance(directory, str):
        directory = Path(directory)
    liv_directory = directory / "LIV"
    if settings is None:
    ...
    for z, file in enumerate(matched_files, start=1):
        print(f"[{z}/{len(matched_files)}] {file}")
        ...
        if "Power conversion efficiency, %" in dataframe.columns:
            ...
            e = dataframe["Power conversion efficiency, %"]
    threshold_index, threshold_current = find_liv_threshold(
        i,
        l,
        liv_threshold_decision_level,
    )
    threshold_voltage = v[threshold_index]
    rollover_index, rollover_current, rollover_output_power = find_liv_rollower(i, l)
    rollover_voltage = v[rollover_index]
    # TODO

    # --------------------------------------------
    # get files for temperature set
    matched_os_files = sorted(liv_directory.glob("*.csv"))
    if matched_os_files:
