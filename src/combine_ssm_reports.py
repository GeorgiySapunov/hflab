...
def combine_ssm_reports_function(path, settings=None):
    ...
    for i, ssm_report_directory in enumerate(list(dict_of_report_directories.keys())):
        ...
        # 1-st row
        ax1_l.plot(df["Current, mA"], df["L, pH"], label=lable)
        ax2_r_p_high.plot(
            df["Current, mA"],
            df["R_p_high, Ohm"],
            label=(lable) + f" (max R_p: {df['R_p_max, Ohm'].max():.2f})",
        )
        ax2_r_p_high.fill_between(
            df["Current, mA"],
            df["R_p_min, Ohm"],
            df["R_p_max, Ohm"],
            color=color,
        ...
        ax22_r_m.plot(df["Current, mA"], df["R_m, Ohm"], label=lable)
        ax23_r_a.plot(df["Current, mA"], df["R_a, Ohm"], label=lable)
        ax3_c_p_low.plot(
            df["Current, mA"],
            df["C_p_low, fF"],
            label=(lable + f" (max C_p: {df['C_p_max, fF'].max():.2f})"),
        )
        ax3_c_p_low.fill_between(
            df["Current, mA"],
            df["C_p_min, fF"],
            df["C_p_max, fF"],
            color=color,
        ...
        ax32_c.plot(df["Current, mA"], df["C_a, fF"], label=lable)
        # 2-nd row
        ax41_f1.plot(df["Current, mA"], df["f1, GHz"], label=lable)
        ax42_f2.plot(df["Current, mA"], df["f2, GHz"], label=lable)
        ax43_f3.plot(df["Current, mA"], df["f3, GHz"], label=lable)
        # 3-nd row
        ax7_gamma.plot(
            df["Current, mA"],
            df["gamma, 1/ns"],
    ...
    ax2_r_p_high.set_ylabel("Resistance, Ohm")
    ...
    ax22_r_m.set_ylabel("Resistance, Ohm")
    ...
    ax23_r_a.set_ylabel("Resistance, Ohm")
