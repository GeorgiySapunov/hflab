...
stop_cond = 0.9  # stop if power lower then 80% of max power
#
#
#
...
def main():
    ...
    if len(sys.argv) != 5:
        ...
        if pm100_toggle:
            ...
            Keysight_8163B.write("*RST")
            Keysight_8163B.write(
                "SENS1:CHAN2:POW:WAV " + wavelength + "nm")  # set wavelength
        ...
        for i in current_list:
            ...
            if i > 0.004:  # if current is more then 4 mA
                if (
        ...
        for i in range(int(current * 10000), 0, -1):  # e.g. 5 mA to 50 and 1 is a step
            ...
            time.sleep(0.02)  # 0.2 sec for a step
        ...
        filepath = (
            ...
            + f"{waferid}-{wavelength}nm-{coordinates}{notes}-{temperature}c-{powermeter}-{timestr}"
        )
